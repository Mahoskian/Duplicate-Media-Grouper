...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Group visually similar media files using pHash (perceptual hash)."
    )
    ...
    parser.add_argument('--hash-size', type=int, default=16,
                        help="Hash grid size (S_HASH). 16 for pHash.")
    parser.add_argument('--threshold', dest='similarity_threshold', type=int, default=25,
...
# === UTILITIES ===
def restore_input_folder(output_dir, input_dir):  # Move files back to input and remove output folder
...
def list_input_files(input_dir, mode):  # Return sorted list of valid media files
...
def compute_phash(array, hash_size):  # Compute perceptual hash bits for image array
    if array.ndim == 3:
...
def average_hash_from_stack(hash_stack, frame_count):  # Average multiple hash vectors
    return (np.sum(hash_stack, axis=0) > (frame_count // 2)).astype(np.uint8)


def hash_image(filename, input_dir, hash_size):  # Hash a single image file
    path = os.path.join(input_dir, filename)
    img = cv2.imread(path)
    if img is None:
        return None
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    return filename, compute_phash(gray, hash_size)


def get_frame_indices(total_frames, sample_count):  # Evenly spaced frame indices
    return set(np.linspace(0, total_frames - 1, sample_count, dtype=int))


def hash_video(filename, input_dir, hash_size, frames_to_sample):  # Hash sampled frames of a video
    path = os.path.join(input_dir, filename)
    try:
        ...
        for idx, frame in enumerate(container.decode(stream)):
            if idx in indices:
                ...
                hashes.append(compute_phash(gray, hash_size))
...
def hash_media(args_tuple):  # Unpack and dispatch to image or video hash
...
def group_similar_files(hash_list, threshold):  # Group by Hamming distance
...
def save_groups(groups, input_dir, output_dir, dry_run):  # Move grouped files
...
def main():  # Main workflow
    ...
    # Prepare iterables for process_map
    it_files = files
