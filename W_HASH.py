...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Group visually similar media files using wHash (wavelet hash)."
    )
    ...
    parser.add_argument('--hash-size', type=int, default=8,
                        help="Hash grid size (S_HASH). 8 for wHash.")
...
def compute_whash(array, hash_size):
    if array.ndim == 3:
...
def hash_image(filename, input_dir, hash_size):
    ...
    return filename, compute_whash(gray, hash_size)
    ...
    try:
        ...
        for idx, frame in enumerate(container.decode(stream)):
            if idx in indices:
                ...
                hashes.append(compute_whash(gray, hash_size))
